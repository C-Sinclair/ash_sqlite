...
defmodule AshSqlite.TenantBinder do
  ...
  `c:bind/2` with that statement's tenant. Reads, aggregates, creates, atomic
  updates, atomic destroys, and bulk operations all go through it, because they all
  go through this data layer.

  ## Contract

  `bind/2` receives the tenant and a zero-arity function, and must return whatever
  the function returns. Whatever it does to select a connection, it must undo:
  ...
  """
  @callback bind(tenant :: term(), fun :: (-> result)) :: result when result: var
end
